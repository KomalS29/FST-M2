-- Load Data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/komal/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/komal/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/komal/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/komal/outputs;

-- Store Result in HDFS
STORE nameOrdered INTO 'hdfs:///user/komal/outputs' USING PigStorage('\t');